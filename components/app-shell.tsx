...
import { BarChart3, Home, Loader2, LogOut, MessageSquareText, Plus, Search, Settings, Shield, Trash2, UsersRound } from 'lucide-react';
import type { Session } from '@supabase/supabase-js';
import { modules, nav, pipeline, roadmap } from '@/lib/data';
import { emptyLeadInput, leadStatuses, normalizeLeadInput, type Lead, type LeadInput, type LeadStatus } from '@/lib/leads';
...
export function AppShell() {
  ...
  if (authLoading) {
    return <main className="premium-grid grid min-h-screen place-items-center"><Loader2 className="animate-spin text-aqua" size={36} /></main>;
  }
...
function LoginScreen({ notice, setNotice }: { notice: Notice; setNotice: (notice: Notice) => void }) {
  ...
  return <main className="premium-grid min-h-screen flex items-center justify-center bg-background px-5">
    <section className="premium-card max-w-md w-full shadow-2xl glass">
      ...
      <form onSubmit={handleLogin} className="mt-8 space-y-4">
        <input className="field" type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="Admin email" required />
        ...
        <button className="w-full rounded-2xl bg-aqua px-6 py-4 font-bold text-obsidian shadow-glow disabled:opacity-60" disabled={loading} type="submit">
...
function DesktopSidebar({ onLogout }: { onLogout: () => void }) { return <aside className="fixed left-6 top-6 z-40 hidden h-[calc(100vh-3rem)] w-60 rounded-[2rem] glass p-5 lg:block">
  ...
  <div className="absolute bottom-5 left-5 right-5 rounded-3xl bg-white/[.08] p-4"><Shield className="mb-3 text-aqua"/><p className="text-sm font-semibold">Enterprise-ready</p><p className="text-xs text-white/50">RBAC, 2FA, modular data boundaries, and scalable integrations.</p></div>
</aside> }

function Hero({ userEmail }: { userEmail: string }) { return <section className="mx-auto max-w-7xl px-4 pb-8 pt-5 sm:px-6 lg:px-8 lg:pt-8"><motion.div {...fade} className="glass rounded-[2rem] p-5 sm:p-8 lg:p-10">
...
function LeadsManager({ setNotice }: { setNotice: (notice: Notice) => void }) {
  ...
  const filteredLeads = useMemo(() => leads.filter((lead) => {
    ...
    const matchesSearch = !term || [lead.name, lead.phone, lead.business_name, lead.email, lead.notes].some((value) => value.toLowerCase().includes(term));
  ...
  useEffect(() => {
    ...
    const channel = client.channel('leads-realtime').on('postgres_changes', { event: '*', schema: 'public', table: 'leads' }, () => {
  ...
  function editLead(lead: Lead) {
    ...
    setForm({ name: lead.name, phone: lead.phone, business_name: lead.business_name, email: lead.email, status: lead.status, notes: lead.notes });
  }
  ...
  return <section id="leads" className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
    ...
    <div className="grid gap-4 xl:grid-cols-[.9fr_1.1fr]">
      <form onSubmit={saveLead} className="glass rounded-[2rem] p-6 space-y-4">
        <h3 className="text-xl font-bold">{editingId ? 'Edit Lead' : 'Add Lead'}</h3>
        <input className="field" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} placeholder="Lead name" required />
        <input className="field" value={form.phone} onChange={(event) => setForm({ ...form, phone: event.target.value })} placeholder="Phone" required />
        <input className="field" value={form.business_name} onChange={(event) => setForm({ ...form, business_name: event.target.value })} placeholder="Business name" required />
        ...
        <select className="field" value={form.status} onChange={(event) => setForm({ ...form, status: event.target.value as LeadStatus })}>{leadStatuses.map((item) => <option key={item} value={item}>{item}</option>)}</select>
        <textarea className="field min-h-28" value={form.notes} onChange={(event) => setForm({ ...form, notes: event.target.value })} placeholder="Notes" />
        <div className="flex gap-3"><button disabled={saving} className="rounded-2xl bg-aqua px-6 py-4 font-bold text-obsidian shadow-glow disabled:opacity-60" type="submit">{saving ? 'Saving...' : editingId ? 'Update Lead' : 'Add Lead'}</button>{editingId ? <button type="button" onClick={() => { setEditingId(null); setForm(emptyLeadInput); }} className="rounded-2xl border border-white/15 px-6 py-4 font-bold text-white">Cancel</button> : null}</div>
      </form>
      <div className="glass rounded-[2rem] p-6">
        <div className="grid gap-3 md:grid-cols-[1fr_220px]"><div className="relative"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-white/40" size={18}/><input className="field pl-11" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search leads" /></div><select className="field" value={status} onChange={(event) => setStatus(event.target.value as 'all' | LeadStatus)}><option value="all">All statuses</option>{leadStatuses.map((item) => <option key={item} value={item}>{item}</option>)}</select></div>
        <div className="mt-5 space-y-3 max-h-[38rem] overflow-y-auto pr-1 no-scrollbar">{loading ? <div className="grid place-items-center py-16"><Loader2 className="animate-spin text-aqua" /></div> : filteredLeads.length ? filteredLeads.map((lead) => <article key={lead.id} className="rounded-3xl bg-white/[.08] p-4"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><h4 className="font-bold">{lead.name}</h4><span className="rounded-full bg-aqua/10 px-3 py-1 text-xs text-aqua">{lead.status}</span></div><p className="mt-1 text-sm text-white/55">{lead.business_name} · {lead.phone} · {lead.email}</p><p className="mt-3 text-sm leading-6 text-white/65">{lead.notes || 'No notes yet.'}</p></div><div className="flex gap-2"><button onClick={() => editLead(lead)} className="rounded-2xl border border-white/15 px-4 py-2 text-sm font-bold">Edit</button><button onClick={() => void deleteLead(lead.id)} className="rounded-2xl bg-red-500/15 px-3 py-2 text-red-200"><Trash2 size={16}/></button></div></div></article>) : <p className="py-16 text-center text-white/50">No leads found.</p>}</div>
      </div>
    </div>
  </section>;
}
...
function NoticeBanner({ notice, inline = false }: { notice: Notice; inline?: boolean }) { if (!notice) return null; return <div className={`${inline ? 'mt-5' : 'fixed right-4 top-4 z-[60]'} rounded-2xl border px-4 py-3 text-sm ${notice.type === 'success' ? 'border-aqua/30 bg-aqua/10 text-aqua' : 'border-red-300/30 bg-red-500/10 text-red-100'}`}>{notice.message}</div>; }
